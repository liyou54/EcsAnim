...
namespace Anim.Editor
{
    public class BakerPrefabDataWindow : OdinEditorWindow
    {
        ...
        // 获取spriteRender数据
        // 这里会产生z-fighting
        public List<SpriteRenderer> GetSpriteRenderers(GameObject skeletonPrefabInstance, out List<EquipInfo> resInfo)
        {
            
            resInfo = new List<EquipInfo>();
            ...
            var res = skeletonPrefabInstance.GetComponentsInChildren<SpriteRenderer>()
                ...
                .ToList();
            var equipArrayInfo = skeletonPrefabInstance.GetComponentInChildren<EquipArrayInfo>().EquipArrayInfos;

            res.RemoveAll((spriteRender) =>
            {
                foreach (var equipInfo in equipArrayInfo)
                {
                    if (equipInfo.EquipArrayInfoEditors.Any(info => info.SpriteRenderer == spriteRender))
                    {
                        return false;
                    }
                }
            ...
            foreach (var equipInfo in equipArrayInfo)
            {
                ...
                info.EquipType = equipInfo.EquipType;
                info.EquipCellInfos = new List<EquipCellInfo>();
                foreach (var spriteRender in equipInfo.EquipArrayInfoEditors)
                {
                    var index = res.IndexOf(spriteRender.SpriteRenderer);
                    info.EquipCellInfos.Add(new EquipCellInfo()
                    {
                        Name = spriteRender.Name,
                        index = index,
                        DefaultSprite = spriteRender.SpriteRenderer.sprite
                    });
                }
                resInfo.Add(info);
            }
        ...
        public void GenMeshBySpriteRender()
        {
            ...
            var spriteRenderers = GetSpriteRenderers(SkeletonPrefabInstance,out var equipInfos);
            ...
            bakedCharacterAsset.EquipInfos = equipInfos;
